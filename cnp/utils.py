...
def get_gp_curve_generator(iterations=10000, batch_size=64, max_num_context=10, testing=False):
    def gen():
        ...
        for _ in range(iterations):
            num_context = tf.random.uniform(shape=[], minval=3, maxval=max_num_context, dtype=tf.int32)
            ...
            if testing is True:
                ...
                num_target = tf.random.uniform(shape=[], minval=2, maxval=max_num_context, dtype=tf.int32)
                num_total_points = num_context + num_target
                x_values = tf.random.uniform((batch_size, num_total_points, 1), minval=-2., maxval=2.)
